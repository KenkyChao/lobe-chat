...
import { resolveDefaultInboxTitle } from '@lobechat/const';
 ...
 * by showing the branded inbox title instead of the agent's own meta.
...
export const useAgentMeta = (messageAgentId?: string | null): MetaData => {
  ...
  return useMemo(() => {
    ...
    if (isBuiltinAgent) {
      // Use DB-stored title if customized (e.g. via onboarding), otherwise fallback to the branded inbox title.
      return { ...agentMeta, title: resolveDefaultInboxTitle(agentMeta.title) };
    }
